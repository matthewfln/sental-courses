...
public class OrderDao implements GenericDao<Order, Integer> {
    ...
    public void save(Order order) {
        ...
        try {
            ...
            em.persist(order);
            tx.commit();
            System.out.println("[OrderDao] Заказ успешно сохранен в БД для клиента: " + order.getCustomerName());
        } catch (Exception e) {
            if (tx.isActive()) {
                tx.rollback();
            }
    ...
    public void update(Order order) {
        ...
        try {
            ...
            em.merge(order);
            tx.commit();
            System.out.println("[OrderDao] Заказ ID " + order.getId() + " успешно обновлен.");
        } catch (Exception e) {
            if (tx.isActive()) {
                tx.rollback();
            }
